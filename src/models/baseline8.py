"""
Two-stage pooled-teams temporal classifier for group activity recognition (baseline8).
...
2. Team pooling + Frame LSTM: max-pools per-team summaries to get frame-level
...
class Two_Stage_Pooled_Teams_Activity_Temporal_Classifier(nn.Module):
    """Two-stage LSTM temporal classifier.

    The architecture:
        - player_lstm: LSTM(input_size -> hidden_size1) applied per player sequence
        - max-pool across the two teams then concatenate to obtain a frame representation
    ...
    def forward(self, x):
        ...
        # ----------------------------------------------------
        # 2. Max-pool teams, and concatenate them -> frame representation
        # ----------------------------------------------------
        team1 = player_out[:, :6, :, :] # (batch, 6, seq_len, hidden_size1)
        team2 = player_out[:, 6:, :, :] # (batch, 6, seq_len, hidden_size1)

        team1 = torch.max(team1, dim=1)[0]
        team2 = torch.max(team2, dim=1)[0]

        clip_out = torch.cat((team1, team2), dim=2) # (batch, seq_len, hidden_size1 * 2)
